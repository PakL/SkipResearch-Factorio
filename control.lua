...
script.on_event(defines.events.on_gui_click, function(event)
	if event.element.name == "skipresearch_yes" then
		skipresearches = true
		game.get_player(event.player_index).force.research_progress = 1
		if game.get_player(event.player_index).gui.top.skipresearch_tbl.skipresearch_btns.skipresearch_mem.state then
			askagain = false
		end
	elseif event.element.name == "skipresearch_all" then
		...
		askagain = false
		game.get_player(event.player_index).force.research_progress = 1
		game.get_player(event.player_index).force.research_all_technologies()
	...
	if event.element.name ~= "skipresearch_mem" then
		for i,plyr in ipairs(game.players) do
